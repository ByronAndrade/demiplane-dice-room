...
function updateAnimatedDice(layer: DiceAnimationLayer, now: number, dt: number): void {
  ...
  for (const die of [...layer.activeDice]) {
    if (!die.settled) {
      ...
      const planeDrag = die.z <= groundZ + 1 ? Math.pow(0.28, dt) : Math.pow(0.58, dt);
      die.vx *= planeDrag;
      die.vy *= planeDrag;
...
function stabilizeDieOnGround(die: AnimatedDie, layer: DiceAnimationLayer, now: number, dt: number): void {
  ...
  die.vx += correctionAxis.y * die.radius * desiredSpin * 0.0055 * rollImpulse * dt;
  die.vy -= correctionAxis.x * die.radius * desiredSpin * 0.0055 * rollImpulse * dt;
